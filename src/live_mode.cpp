...
private:
    ...
    void stop_recording_and_transcribe() {
        ...
        worker_ = std::thread([this]() {
            ...
            if (streaming_enabled_for_session_ && stream_session_) {
                ...
                log.stream_agreement_n = stream_result.stream_agreement_n;
                log.stream_history_depth = stream_result.stream_history_depth;
                log.stream_hypothesis_chars = stream_result.stream_hypothesis_chars;
                log.stream_confirmed_prefix_tokens = stream_result.stream_confirmed_prefix_tokens;
                log.stream_commit_advance_tokens = stream_result.stream_commit_advance_tokens;
                log.stream_unconfirmed_suffix_chars = stream_result.stream_unconfirmed_suffix_chars;
                log.stream_prompt_tokens = stream_result.stream_prompt_tokens;
                log.stream_audio_trim_ms = stream_result.stream_audio_trim_ms;
                log.stream_buffer_base_ms = stream_result.stream_buffer_base_ms;
                log.stream_local_agreement_match_tokens = stream_result.stream_local_agreement_match_tokens;
                log.stream_segment_trim_used = stream_result.stream_segment_trim_used;
                log.stream_waited_for_inflight_decode = stream_result.waited_for_inflight_decode;
                log.stream_finalization_source = stream_result.stream_finalization_source;
                ...
                debug_line("[STREAM_AGREEMENT_N] " + std::to_string(log.stream_agreement_n));
                debug_line("[STREAM_HISTORY_DEPTH] " + std::to_string(log.stream_history_depth));
                debug_line("[STREAM_HYPOTHESIS_CHARS] " + std::to_string(log.stream_hypothesis_chars));
                debug_line("[STREAM_CONFIRMED_PREFIX_TOKENS] " + std::to_string(log.stream_confirmed_prefix_tokens));
                debug_line("[STREAM_COMMIT_ADVANCE_TOKENS] " + std::to_string(log.stream_commit_advance_tokens));
                debug_line("[STREAM_UNCONFIRMED_SUFFIX_CHARS] " + std::to_string(log.stream_unconfirmed_suffix_chars));
                debug_line("[STREAM_PROMPT_TOKENS] " + std::to_string(log.stream_prompt_tokens));
                debug_line("[STREAM_AUDIO_TRIM_MS] " + std::to_string(log.stream_audio_trim_ms));
                debug_line("[STREAM_BUFFER_BASE_MS] " + std::to_string(log.stream_buffer_base_ms));
                debug_line(std::string("[STREAM_SEGMENT_TRIM_USED] ") + (log.stream_segment_trim_used ? "true" : "false"));
                debug_line("[STREAM_LOCAL_AGREEMENT_MATCH_TOKENS] " + std::to_string(log.stream_local_agreement_match_tokens));
    ...
    void append_log(const std::filesystem::path& log_path, const LogPayload& log) {
        ...
        out << "[STREAM_AGREEMENT_N] " << log.stream_agreement_n << "\n";
        out << "[STREAM_HISTORY_DEPTH] " << log.stream_history_depth << "\n";
        out << "[STREAM_HYPOTHESIS_CHARS] " << log.stream_hypothesis_chars << "\n";
        out << "[STREAM_CONFIRMED_PREFIX_TOKENS] " << log.stream_confirmed_prefix_tokens << "\n";
        out << "[STREAM_COMMIT_ADVANCE_TOKENS] " << log.stream_commit_advance_tokens << "\n";
        out << "[STREAM_UNCONFIRMED_SUFFIX_CHARS] " << log.stream_unconfirmed_suffix_chars << "\n";
        out << "[STREAM_PROMPT_TOKENS] " << log.stream_prompt_tokens << "\n";
        out << "[STREAM_AUDIO_TRIM_MS] " << log.stream_audio_trim_ms << "\n";
        out << "[STREAM_BUFFER_BASE_MS] " << log.stream_buffer_base_ms << "\n";
        out << "[STREAM_SEGMENT_TRIM_USED] " << (log.stream_segment_trim_used ? "true" : "false") << "\n";
        out << "[STREAM_LOCAL_AGREEMENT_MATCH_TOKENS] " << log.stream_local_agreement_match_tokens << "\n";
        out << "[STREAM_FINALIZATION_SOURCE] " << (log.stream_finalization_source.empty() ? "local_agreement" : log.stream_finalization_source)
            << "\n";

...
bool StreamingDictationSession::start(uint64_t session_id) {
    ...
    capture_start_total_samples_ = capture_.get_total_captured_samples();
...
void StreamingDictationSession::worker_loop() {
    ...
    while (!stop_requested_.load()) {
        ...
        const std::size_t total_captured_samples_abs = capture_.get_total_captured_samples();
        const std::size_t total_captured_samples =
            total_captured_samples_abs > capture_start_total_samples_ ? (total_captured_samples_abs - capture_start_total_samples_) : 0;

        std::vector<float> rolling_pcm;
        capture_.get_latest_pcm_ms(30000, rolling_pcm);
        if (rolling_pcm.empty()) {
            continue;
        }
        const std::size_t rolling_end_sample = total_captured_samples;
        const std::size_t rolling_start_sample =
            rolling_pcm.size() > rolling_end_sample ? 0 : (rolling_end_sample - rolling_pcm.size());

        int iteration = 0;
        std::size_t buffer_base_sample_snapshot = 0;
        std::vector<int> prompt_tokens_snapshot;
        {
            std::lock_guard<std::mutex> lock(mutex_);
            ++decode_iteration_count_;
            iteration = decode_iteration_count_;
            buffer_base_sample_snapshot = audio_buffer_base_samples_;
            prompt_tokens_snapshot = committed_prompt_tokens_;
            committed_prompt_tokens_.clear();
            decode_in_progress_ = true;
        }

        const std::size_t decode_start_sample = (std::max)(rolling_start_sample, buffer_base_sample_snapshot);
        if (decode_start_sample >= rolling_end_sample) {
            continue;
        }
        const std::size_t decode_offset = decode_start_sample - rolling_start_sample;
        std::vector<float> window_pcm(rolling_pcm.begin() + static_cast<std::ptrdiff_t>(decode_offset), rolling_pcm.end());
        if (window_pcm.empty()) {
            continue;
        }
        ...
        const int decode_start_ms = static_cast<int>((decode_start_sample * 1000) / AudioCapture::kSampleRate);
        hypothesis.audio_buffer_start_ms = decode_start_ms;
        hypothesis.audio_buffer_end_ms = decode_start_ms + window_ms;
                ...
                if (trim_on_segment_boundary_) {
                    ...
                    for (const auto& segment : latest.segments) {
                        ...
                        if (committed_prefix_tokens_normalized_.size() >= committed_boundary_tokens) {
                            const int segment_end_abs_ms = latest.audio_buffer_start_ms + segment.t1_ms;
                            candidate_trim_ms = (std::max)(candidate_trim_ms, segment_end_abs_ms);
                    ...
                    if (guarded > audio_buffer_base_ms_) {
                        ...
                        audio_buffer_base_samples_ =
                            static_cast<std::size_t>((static_cast<int64_t>(audio_buffer_base_ms_) * AudioCapture::kSampleRate) / 1000);
                    }
                }

...
void StreamingDictationSession::worker_loop() {
    ...
    while (!stop_requested_.load()) {
...

        {
            ...
            const ConfirmedPrefixResult agreement =
                compute_confirmed_prefix(hypothesis_history_, committed_prefix_tokens_normalized_);
            ...
            if (agreement.newly_confirmed_tokens > 0 && !hypothesis_history_.empty()) {
                ...
                const std::size_t end = start + agreement.newly_confirmed_tokens;
                ...
                committed_prefix_tokens_normalized_.insert(committed_prefix_tokens_normalized_.end(),
                                                           ...
                                                           latest.tokens_normalized.begin() + static_cast<std::ptrdiff_t>(end));
                ...
                if (static_cast<int>(committed_prompt_tokens_.size()) > prompt_max_tokens_) {
                    committed_prompt_tokens_.erase(
                        ...
                        committed_prompt_tokens_.begin() + (static_cast<std::ptrdiff_t>(committed_prompt_tokens_.size()) - prompt_max_tokens_));
                }
                ...
                if (trim_on_segment_boundary_) {
                    ...
                    for (const auto& segment : latest.segments) {
                        ...
                        std::vector<std::string> segment_original;
                        std::vector<std::string> segment_normalized;
                        tokenize_text(segment.text, segment_original, segment_normalized);
                        if (segment_normalized.empty()) {
                            continue;
                        }
                        const std::size_t seg_end = segment_normalized.size();
                        if (committed_prefix_tokens_normalized_.size() >= seg_end) {
